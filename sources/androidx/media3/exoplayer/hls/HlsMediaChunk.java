...
final class HlsMediaChunk extends MediaChunk {
    ...
    public static HlsMediaChunk createInstance(HlsExtractorFactory hlsExtractorFactory, DataSource dataSource, Format format, long j, HlsMediaPlaylist hlsMediaPlaylist, HlsChunkSource.SegmentBaseHolder segmentBaseHolder, Uri uri, List<Format> list, int i, Object obj, boolean z, TimestampAdjusterProvider timestampAdjusterProvider, long j2, HlsMediaChunk hlsMediaChunk, byte[] bArr, byte[] bArr2, boolean z2, PlayerId playerId2, CmcdHeadersFactory cmcdHeadersFactory) {
        ...
        if (cmcdHeadersFactory2 == null) {
            ...
            immutableMap = cmcdHeadersFactory2.setChunkDurationUs(segmentBase.durationUs).createHttpRequestHeaders();
        }
        DataSpec build = new DataSpec.Builder().setUri(UriUtil.resolveToUri(hlsMediaPlaylist2.baseUri, segmentBase.url)).setPosition(segmentBase.byteRangeOffset).setLength(segmentBase.byteRangeLength).setFlags(segmentBaseHolder2.isPreload ? 8 : 0).setHttpRequestHeaders(immutableMap).build();
        ...
        if (segment != null) {
            ...
            Uri resolveToUri = UriUtil.resolveToUri(hlsMediaPlaylist2.baseUri, segment.url);
            if (cmcdHeadersFactory2 == null) {
                immutableMap2 = ImmutableMap.of();
            } else {
                immutableMap2 = cmcdHeadersFactory2.setObjectType(CmcdHeadersFactory.OBJECT_TYPE_INIT_SEGMENT).createHttpRequestHeaders();
            }
            dataSpec = new DataSpec.Builder().setUri(resolveToUri).setPosition(segment.byteRangeOffset).setLength(segment.byteRangeLength).setHttpRequestHeaders(immutableMap2).build();
        ...
        if (hlsMediaChunk2 != null) {
            ...
            boolean z6 = dataSpec == dataSpec2 || (dataSpec != null && dataSpec2 != null && dataSpec.uri.equals(hlsMediaChunk2.initDataSpec.uri) && dataSpec.position == hlsMediaChunk2.initDataSpec.position);
            ...
            hlsMediaChunkExtractor = (!z6 || !z7 || hlsMediaChunk2.extractorInvalidated || hlsMediaChunk2.discontinuitySequenceNumber != i2) ? null : hlsMediaChunk2.extractor;
        } else {
            Uri uri2 = uri;
