...
import stikerwap.appdys.t_buscador_fr;

public class t_buscador extends FragmentActivity_ext_class implements Activity_ext, t_buscador_fr.OnTutSelectedListener, View.OnClickListener, RewardedVideoAdListener, OnAdLoaded, OnAdClosed, OnVideoEnded, OnAdError, VideoListener, AdEventListener {
    Anuncios anun;
    ...
    public void onCreate(Bundle bundle) {
        ...
        this.viewer = (t_detalle_fr) getSupportFragmentManager().findFragmentById(R.id.detalle_fr);
        incluir_menu_pre();
        if (z2) {
        ...
        searchManager.setOnCancelListener(new SearchManager.OnCancelListener() {
            public void onCancel() {
                t_buscador.this.finalizar = false;
                t_buscador.this.setResult(0);
            }
        });
        searchManager.setOnDismissListener(new SearchManager.OnDismissListener() {
            public void onDismiss() {
                t_buscador.this.buscador_on = false;
            }
        });
        if (bundle == null) {
    ...
    public void onTutSelected(String str, long j) {
        t_detalle_fr t_detalle_fr = this.viewer;
        if (t_detalle_fr == null || !t_detalle_fr.isInLayout()) {
        ...
        this.viewer.actContenido(str, j);
    }
    ...
    private void establec_ralc(final Context context) {
        this.ralc = new RewardedAdLoadCallback() {
            public void onAdFailedToLoad(LoadAdError loadAdError) {
                if (!t_buscador.this.globales.admob_rew_failed(context, t_buscador.this.mAd_appnext)) {
                    t_buscador.this.dialog_cargando.cancel();
                    t_buscador t_buscador = t_buscador.this;
                    t_buscador.abrir_secc(t_buscador.v_abrir_secc);
                }
            }
            ...
            public void onAdLoaded(RewardedAd rewardedAd) {
                ...
                rewardedAd.setFullScreenContentCallback(new FullScreenContentCallback() {
                    ...
                    public void onAdFailedToShowFullScreenContent(AdError adError) {
                        t_buscador.this.abrir_secc(t_buscador.this.v_abrir_secc);
                    }

                    public void onAdDismissedFullScreenContent() {
                        if (t_buscador.this.mAd_visto) {
                            t_buscador.this.abrir_secc(t_buscador.this.v_abrir_secc);
                        }
                    }
                });
                rewardedAd.show((Activity) context, new OnUserEarnedRewardListener() {
                    public void onUserEarnedReward(RewardItem rewardItem) {
                        t_buscador.this.mAd_visto = true;
